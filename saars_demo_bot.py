...
try:
    from menu_builder import MENU_EXACT, build_keyboard, register_menu
    _MENU_BUILDER = True
except ImportError:
    _MENU_BUILDER = False
    MENU_EXACT = {}
...
async def cmd_start(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    # Usa InlineKeyboardButton explícito com callback_data (nunca url=)
    await u.message.reply_html(
        ...
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("🚀 Abrir Demo", callback_data="demo:main")
        ]])
    )
...
# Merge handlers do menu_builder (sobrescreve demo:main, demo:sobre, demo:fechar)
EXACT.update(MENU_EXACT)
...
async def router(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    for pfx, fn in PREFIX.items():
        if data.startswith(pfx):
            return await fn(u, c)
    # Callback desconhecido — responde para evitar spinner infinito no Telegram
    await q.answer("⚠️ Ação não reconhecida.", show_alert=False)
...
async def run():
    ...
    async with app:
        ...
        await asyncio.Event().wait()  # mantém vivo indefinidamente
