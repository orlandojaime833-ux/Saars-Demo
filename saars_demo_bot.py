...
async def cmd_start(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    await u.message.reply_html(
        ...
        "👇 Clica para começar:",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🚀 Abrir Demo", callback_data="demo:main")]])
    )
...
async def prod_detail(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if ok:
        ...
        rows = [
            [InlineKeyboardButton("📦 Ver entrega", callback_data="demo:redeliver:"+str(pid))],
            [InlineKeyboardButton("🔙 Loja", callback_data="demo:loja")],
        ]
    else:
        rows = []
        if b >= p["price"]: rows.append([InlineKeyboardButton(f"💰 Comprar com saldo ({b:.2f} USDT)", callback_data="demo:buybal:"+str(pid))])
        rows.append([InlineKeyboardButton("💳 Pagar com crypto (demo)", callback_data="demo:buycrypto:"+str(pid))])
        rows.append([InlineKeyboardButton("🔙 Loja", callback_data="demo:loja")])
...
async def task_detail(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if done:
        txt += "\n\n✅ <b>Já concluída!</b>"
        rows = [[InlineKeyboardButton("🔙 Tarefas", callback_data="demo:tarefas")]]
    else:
        ...
        rows = [
            ...
            [InlineKeyboardButton("🔙 Tarefas", callback_data="demo:tarefas")],
        ]
    await u.callback_query.edit_message_text(txt, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(rows))
...
async def guard(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    rows += [
        [InlineKeyboardButton("✅ Verificar adesão (demo)", callback_data="demo:guard_ok")],
...
async def run():
    # HTTP keepalive para o Render não matar o processo
